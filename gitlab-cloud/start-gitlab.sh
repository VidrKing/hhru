...
#Создаём отдельный каталог под GitLab

FOLDER_NAME="$TASK_ID""-gitlab-folder"
...
SA_NAME="$TASK_ID""-gitlab-terraform-sa"
...
yc iam key create \
    --service-account-name "$TASK_ID""-gitlab-terraform-sa" \
